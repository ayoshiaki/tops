...
namespace tops {
  ...
  void PairHiddenMarkovModel::initialize(const ProbabilisticModelParameters & parameters) {
	...
	exit(-1);
      ...
      if(eit == emiss.end()){
	int id1 = observations->getSymbol(symbol[0])->id();
	int id2 = observations->getSymbol(symbol[1])->id();
  ...
  std::string PairHiddenMarkovModel::getStateName(int state) const{
    return _hmm->getStateName(state);
  }

  /*Sequence &  PairHiddenMarkovModel::chooseObservation ( Sequence & h, int i, int state) const
  {
    return _hmm->chooseObservation(h,i,state);
  }

  int PairHiddenMarkovModel::chooseState(int state) const
  {
    return _hmm->chooseState(state);
  }

  int PairHiddenMarkovModel::chooseFirstState() const
  {
    return _hmm->chooseFirstState();
  }
  */

  void PairHiddenMarkovModel::setInitialProbability(MultinomialDistributionPtr initial) {
    _hmm->setInitialProbability(initial);
  }

  void PairHiddenMarkovModel::setObservationSymbols(AlphabetPtr obs) {
    tops::ProbabilisticModel::setAlphabet(obs);
  }
  void PairHiddenMarkovModel::setStates(std::vector<HMMStatePtr> states, AlphabetPtr state_names) {
    _hmm->setStates(states,state_names);
  }

  double PairHiddenMarkovModel::getEmission(const Sequence & seq1, const Sequence & seq2, int state_id, int seq1Pos, int seq2Pos, int* state_type) const{
    double match, gap1, gap2;
    MultinomialDistributionPtr emiss;
    emiss = getState(state_id)->emission();
    match = emiss->log_probability_of_pair(seq1[seq1Pos-1],seq2[seq2Pos-1]);
    gap1 = emiss->log_probability_of_pair(seq1[seq1Pos-1], _gap_id);
    gap2 = emiss->log_probability_of_pair(_gap_id, seq2[seq2Pos-1]);
  ...
  double PairHiddenMarkovModel::forward(const Sequence & seq1, const Sequence & seq2, vector<Matrix> &a) const
  {
    int *state_type;
    int nstates = (_hmm->getStates()).size();
    ...
    //This is not correct in some cases. 
    //We dont treat the cases where there is more than one state that can generate observations in only one sequence.
    //We are not considering silent states.
    for(int k = 0; k < nstates; k++){
      alpha[k](0,0) = _hmm->getInitialProbabilityOf(k);
      for (int j = 1; j <= length1; j++)
	alpha[k](j,0) = alpha[k](j-1,0) + getState(k)->transitions()->log_probability_of(k) + getState(k)->emission()->log_probability_of(seq1[j-1], _gap_id);
      for (int j = 1; j <= length2; j++)
	alpha[k](0,j) = alpha[k](0,j-1) + getState(k)->transitions()->log_probability_of(k) + getState(k)->emission()->log_probability_of(_gap_id, seq2[j-1]);
    }

    for (int i = 1; i <= length1; i++){
      for(int j = 1; j <= length2; j++){
	for(int k = 0; k < nstates; k++){
	  alpha[k](i,j) = getEmission(seq1,seq2,k,i,j,state_type);
	  if(*state_type == 1){
	    double aux = 0;
	    for(int l = 0; l < nstates; l++)
	      aux = log_sum(aux, alpha[l](i-1,j-1) + getState(l)->transitions()->log_probability_of(k));
	    alpha[k](i,j) += aux;
	  }
	  else if(*state_type == 2){
	    double aux = 0;
	    for(int l = 0; l < nstates; l++)
	      aux =  log_sum(aux, alpha[l](i-1,j) + getState(l)->transitions()->log_probability_of(k));
	    alpha[k](i,j) += aux;
	  }
	  else{
	    double aux = 0;
	    for(int l = 0; l < nstates; l++)
	      aux =  log_sum(aux, alpha[l](i,j-1) + getState(l)->transitions()->log_probability_of(k));
	    alpha[k](i,j) += aux;
	  }
	}
      }
    ...
    a = alpha;
    double sum = alpha[0](length1,length2);
	...
	/*  double HiddenMarkovModel::backward(const Sequence & sequence, Matrix &b) const
  {
    ...
    //! Viterbi algorithm
  double PairHiddenMarkovModel::viterbi (const Sequence &seq1, cont Sequence &seq2, Sequence &path, Matrix & viterbi) const 
  {
