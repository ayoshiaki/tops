...
namespace tops {
  ...
  void GeneralizedPairHiddenMarkovModel::initialize(const ProbabilisticModelParameters & parameters) {
    ...
    MultinomialDistributionPtr init = MultinomialDistributionPtr(new MultinomialDistribution());
	...
	t = MultinomialDistributionPtr(new MultinomialDistribution(strdvit->second));
      else{
